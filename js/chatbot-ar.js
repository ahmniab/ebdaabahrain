 ...
 function sanitizeInput(input) {
  ...
  function processMessage(message) {
    ...
    if (lowerMsg.includes('قرض') || lowerMsg.includes('اقتراض')) {
      showLoanMenu();
    } 
...
function startListening() {
  ...
  function showGreeting() {
    ...
    insertBotMessage(`
      ${greeting} 👋<br>
      أنا مساعدك الذكي من الإبداع للتمويل متناهي الصغر. كيف يمكنني مساعدتك اليوم؟
  ...
  function showLoanMenu() {
    ...
    Object.keys(loans).forEach(loan => {
      html += `
        <button class="quick-btn" onclick="showLoanDetails('${loan}')" aria-label="عرض تفاصيل ${loans[loan].title}">
