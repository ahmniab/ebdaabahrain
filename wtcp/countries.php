...
<div align="center">
  <p align="center"><strong><font color="#006600" face="Arial, Helvetica, sans-serif"><br>
        </font><font color="#006600"><font color="006699">الدول</font></font></strong></p>
  <p>
...
?>
 ...
 <form action='<?php echo $_SERVER['PHP_SELF'];?>?action=get_add' method='post' name='countries' id='countries'>
    <table width='450' border='0' align='center' cellpadding='1'>
      <tr>
        <td><table width='500' border='0' cellpadding='1'>
            <tr align='left' valign='top'>
              <td width="153"><font color="#006699" face="Arial, Helvetica, sans-serif">&#1571;&#1587;&#1605; &#1575;&#1604;&#1583;&#1608;&#1604;&#1577;</font></td>
...
// ck user if exist
$query = MySQL_Query("SELECT id FROM countries WHERE ar_name='{$_POST['ar_name']}'") Or Die(MySQL_Error()); 
...
// add to admin table
$in = MySQL_Query("INSERT INTO countries (name,ar_name,code) VALUES ('$name', '{$_POST['ar_name']}', '{$_POST['code']}')") 
Or Die(MySQL_Error()); 
if(!$in) 
{
echo "
...
$edit_countries = mysql_query("select * from countries where id='{$_GET['id']}' ");

/// cer
while($edit = mysql_fetch_array($edit_countries)){ 
extract($edit);
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>?action=update" method="post" name="countries" id="countries">
    <table width="450" border="0" align="center" cellpadding="1">
      <tr>
        <td><table width="500" border="0" cellpadding="1">
            <tr align="left" valign="top">
              <td width="153"><font color="006699">&#1571;&#1587;&#1605; &#1575;&#1604;&#1583;&#1608;&#1604;&#1577;</font></td>
...
// add to admin table
$in = mysql_query (" UPDATE countries SET name='$name' , ar_name='$ar_name', code='$code' WHERE id='{$_POST['id']}' ") 
Or Die(MySQL_Error()); 
if(!$in) 
{
echo "
...
$del_countries = mysql_query("select * from countries where id='{$_GET['id']}' ");

/// cer
while($del = mysql_fetch_array($del_countries)){ 
    extract($del);
	$name = stripslashes ("$name");
...
//// result
$in = mysql_query (" DELETE FROM countries WHERE id='{$_POST['id']}' ") 
Or Die(MySQL_Error()); 
// data input
echo " 
...
$view_all = mysql_query("select * from users WHERE country='$id' ORDER BY username ASC LIMIT $from, $max_results")Or Die(MySQL_Error()); 
...
$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM users WHERE country='$id'"),0)Or Die(MySQL_Error()); 
...
$view_all = mysql_query("SELECT * FROM countries ORDER BY ar_name ASC LIMIT $from, $max_results")Or Die(MySQL_Error()); 
...
/////////////////////////
// Figure out the total number of results in DB: 
$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM countries "),0)Or Die(MySQL_Error()); 
